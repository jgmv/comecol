...
#' @param phy the name of the file with a phylogenetic tree in Newick format including the species in 'cdm'. Tip labels must be the species names as in columns of 'cdm'. If no tree is provided, a taxonomic tree is automatically generated using the information in 'tax'.
...
read_com <- function(cdm, env, tax, seq = NULL, phy = NULL, file = NULL,
  ...
  if(use_saved & file.exists(file)){    
    ...
    if(is.null(phy)) com$phy <- taxtotree(com) 
