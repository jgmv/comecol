...
#' @details Eigenvectors are obtained from a Principal Coordinates Analysis calculated from the bioclimatic data.
#' @return A list with community data.
#' @keywords community data
#' @export
#' @examples
#' clim_data()
clim_data <- function(com, x = "lon", y = "lat", k = 3, keep_data = T) {
  ...
  if(!keep_data) unlink("wc10", recursive = T)
    ...
    com$climate_eig <- bio_coord
    com$climate_pca <- bio_pca
